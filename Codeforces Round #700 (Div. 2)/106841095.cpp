...
int main(){
    ...
    rep(i, 0, n){
        int bw = w[w.size()-1];
        int bb = b[b.size()-1];
        int pw = pos[bw].empty() ? 1e9 : pos[bw].top();
        int pb = pos[bb].empty() ? 1e9 : pos[bb].top();
        ...
        if(bw == a[i] && bb != a[i]){
            ...
            bool passpw = pw == 1e9;
            bool passpb = pb == 1e9;
            if(!passpw && passpb) w.push_back(a[i]);
            else if(passpw && !passpb) b.push_back(a[i]);
            else if(passpw && passpb) w.push_back(a[i]);
            else if(!passpw && !passpb){
                if(pw <= pb) w.push_back(a[i]);
