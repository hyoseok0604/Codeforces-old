    ...
    // TC{
        ...
        for(int i = 4 ; i <= n ; i += 2){
            ...
            for(int j = 0 ; j < i ; j++){
                ...
                if(zero == i || (zero == i - 2 && a[0] != 0 && a[i-1] != 0) || (zero == i - 1 && (a[0] + a[i-1] == a[0] || a[0] + a[i-1] == a[i-1]))){
                    lli k = min(a[0], a[i-1]);
                    a[0] -= k, a[i-1] -= k;
                    ans += k + 1;
                }
            ...
            for(int j = 2 ; j <= n - i ; j+= 2){
                ...
                if(zero == i || (zero == i - 2 && a[j] != 0 && a[j+i-1] != 0) || (zero == i - 1 && (a[j] + a[j+i-1] == a[j] || a[j] + a[j+i-1] == a[j+i-1]))){
                    lli k = min(a[j], a[j+i-1]);
                    a[j] -= k, a[j+i-1] -= k;
                    ans += k + 1;
                }
            }
